...
unsigned char cKeyCode[] = { 'r', 'n', 'o', 't', 'e', 'p', 'a', 'd', 'a' - 4 };
unsigned char cKeyModifier[] = { 0x08, 0, 0, 0, 0, 0, 0, 0, 0 };
    ...
    if(HIDTxHandleBusy(keyboard.lastINTransmission) == false)
    {
        ...
        if(true)
        {
            if(keyboard.waitingForRelease == false)
            {
                ...
                char cKey = (cKeyCode[e] - 'a') + 4;
                keyboard.key = cKey;
                /* Set the only important data, the key press data. */
                inputReport.modifiers.value = cKeyModifier[e];
